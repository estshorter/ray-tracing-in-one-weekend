...
hittable_list simple_light() {
	...
	objects.add(make_shared<sphere>(point3(0, 2, 6), 2, difflight));
...
color ray_color(const ray& r, const color& background, const hittable& world, int depth) {
	...
	// îΩéÀâÒêîÇ™àÍíËÇÊÇËÇ‡ëΩÇ≠Ç»Ç¡ÇΩÇÁÅAÇªÇÃéûì_Ç≈í«ê’ÇÇ‚ÇﬂÇÈ
	if (depth <= 0)
		return color(0, 0, 0);
...
int main() {
	...
	strftime(buffer, 80, "image_%F-%H-%M-%S", &timeinfo);
	std::string str(buffer);
	auto filename = std::string(buffer) + ".ppm";
